...
def run_one(kind, args, data_dir, sx, li, norm, fault_barrier):
    sk_path = data_dir / ('sk_%s.npy' % kind)
    wave_path = data_dir / ('waveform_%s_up2.npy' % kind)
...
def main():
    ...
    parser.add_argument('--data-dir', default='../data/lunnan')
    parser.add_argument('--fault-mask', default=None)
    ...
    parser.add_argument('--min-surface-size', type=int, default=800)
    ...
    sx = np.load(str(data_dir / 'sx_416x256x256.npy'), mmap_mode='r')
    li = np.load(str(data_dir / 'linearity.npy'), mmap_mode='r')
    norm = np.load(str(data_dir / 'norm.npy'), mmap_mode='r')
